...
function DashboardContent() {
  ...
  const filteredStores = useMemo(() => Array.isArray(stores) ? stores.filter(store => {
    const matchesSearch = store.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
        store.location.toLowerCase().includes(searchQuery.toLowerCase());

    const matchesBrand =
        selectedBrand === 'all' ||
        (selectedBrand === '1' && (store.brandId === 2 || store.brandId === 3)) ||
  ...
  }) : [], [stores, searchQuery, selectedBrand, selectedMediaItemTypeId]);
